...
// ------ with unsafe buffers
pub(super) static VEC_UNSAFE:OnceLock<super::MutBuf> = OnceLock::new(); // 'real' array
pub(super) static BUF_UNSAFE:OnceLock<super::MutBuf> = OnceLock::new(); // buffer for reusable scratch space

pub(super) fn sort_unsafe(left: usize, right: usize, usebuf: bool ) {
    let len = right-left;

    if usebuf && len <= SORT_CHUNK {
        let vec = VEC_UNSAFE.get().unwrap();
        let slice = vec.as_mut_slice(left, right);
        slice.sort();
    } else {
        let mid = len / 2;
        sort_unsafe(left, left+mid, !usebuf);
        sort_unsafe(left+mid, right, !usebuf);
        
        merge_unsafe((left, left+mid), (left+mid, right), (left, right), usebuf);
    }
}

#[cfg(not(feature = "test_direct_rec"))]
#[spawnable]
pub(super) fn spawn_sort_unsafe(left: usize, right: usize, usebuf: bool ) {
    let len = right-left;
    let mid = len / 2;
    ...
    if usebuf && len <= SORT_CHUNK {
        ...
        spawn_sort_unsafe(left, left+mid, !usebuf);
        spawn_sort_unsafe(left+mid, right, !usebuf);
    }
    // TODO: no automatic sync detection possible here !!!! need a sync!() macro
    spawn_merge_unsafe(__worker__, (left, left+mid), (left+mid, right), (left, right), usebuf); // TODO: manually added __worker__

}

fn merge_unsafe ((mut a_left, mut a_right): (usize, usize), (mut b_left, mut b_right): (usize, usize), dest: (usize, usize), invec: bool) {
    // let 'a' be the larger of the two sub-arrays
    ...
    if dest.1 - dest.0 <= MERGE_CHUNK {
        merge_seq_unsafe((a_left, a_right), (b_left, b_right), dest, invec);
        return;
    }
    
    let (left, right) = if invec {
        (BUF_UNSAFE.get().unwrap().as_slice(a_left, a_right),
        BUF_UNSAFE.get().unwrap().as_slice(b_left, b_right))
    } else {
        (VEC_UNSAFE.get().unwrap().as_slice(a_left, a_right),
        VEC_UNSAFE.get().unwrap().as_slice(b_left, b_right))
    };

    // find the middle element of left, and use std's binary_search to find suitable index in right
    let a_mid = left.len() / 2;
    let b_mid = match right.binary_search(&left[a_mid]) {
        Ok(i) => i,
        Err(i) => i,
    };

    // recurse, splitting at the newly found 'mid' indexes (in the sense of value, not array size)
    merge_unsafe((a_left, a_left + a_mid), (b_left, b_left+b_mid), (dest.0, dest.0+a_mid+b_mid), invec);
    merge_unsafe((a_left + a_mid, a_right), (b_left + b_mid, b_right), (dest.0+a_mid+b_mid, dest.1), invec);
}

#[cfg(not(feature = "test_direct_rec"))]
#[spawnable]
pub(super) fn spawn_merge_unsafe ((mut a_left, mut a_right): (usize, usize), (mut b_left, mut b_right): (usize, usize), dest: (usize, usize), invec: bool) {
    // let 'a' be the larger of the two sub-arrays
    if (a_right - a_left) < (b_right - b_left) { 
        let (tmp_left, tmp_right) = (a_left, a_right);
        (a_left, a_right) = (b_left, b_right);
    ...
    // recurse, splitting at the newly found 'mid' indexes (in the sense of value, not array size)
    spawn_merge_unsafe((a_left, a_left + a_mid), (b_left, b_left+b_mid), (dest.0, dest.0+a_mid+b_mid), invec);
    spawn_merge_unsafe((a_left + a_mid, a_right), (b_left + b_mid, b_right), (dest.0+a_mid+b_mid, dest.1), invec);
}

fn merge_seq_unsafe ((a_left, a_right): (usize, usize), (b_left, b_right): (usize, usize), (d_left, d_right): (usize, usize), invec: bool) {
    let (src_left, src_right, dest) = if invec {
        (BUF_UNSAFE.get().unwrap().as_slice(a_left, a_right),
        BUF_UNSAFE.get().unwrap().as_slice(b_left, b_right),
        VEC_UNSAFE.get().unwrap().as_mut_slice(d_left, d_right)) 
    } else {
        (VEC_UNSAFE.get().unwrap().as_slice(a_left, a_right),
        VEC_UNSAFE.get().unwrap().as_slice(b_left, b_right),
        BUF_UNSAFE.get().unwrap().as_mut_slice(d_left, d_right))
    };
...
#[cfg(feature = "test_direct_rec")]
pub(super) fn spawn_sort_unsafe(
    __worker__: &mut velvet::VelvetWorker<crate::__Frame__>,
    left: usize,
    right: usize,
    usebuf: bool,
    ...
    if usebuf && len <= SORT_CHUNK {
        ...
        __worker__
            .spawn(
                crate::__Frame__::InputSpawnSortUnsafe(
                    __0__,
                    left,
                    left + mid,
                    !usebuf,
...
#[cfg(feature = "test_direct_rec")]
pub(super) fn spawn_merge_unsafe(
    __worker__: &mut velvet::VelvetWorker<crate::__Frame__>,
    (mut a_left, mut a_right): (usize, usize),
    (mut b_left, mut b_right): (usize, usize),
    dest: (usize, usize),
    ...
    let __SYNC__ = __worker__.sync(__1__);
    let __SYNC_RES__ = match __SYNC__ {
        crate::__Frame__::InputSpawnMergeUnsafe(_, a0, a1, a2, a3) => {
            spawn_merge_unsafe(__worker__, a0, a1, a2, a3)
        }
        crate::__Frame__::Stolen(ptr) => {
            let mut try_lock = ptr.try_lock();
            loop {
