...
echo "NQUEENS benchmark. Saving logs to $OUT"
...
if [ "$HAVE_RUST" = "true" ]; then
    ...
    echo "running NQUEENS serial Rust"
    for iter in "${ACTIVE_ITERS[@]}"; do
        taskset -c 0 cargo run --release seq $N >> "$OUT" 2>> "$DUMP"
    done

    echo "running NQUEENS serial elision"
    for iter in "${ACTIVE_ITERS[@]}"; do
        taskset -c 0 cargo run --release par $N >> "$OUT" 2>> "$DUMP"
    done

    echo "running NQUEENS rayon"
    for threads in "${ACTIVE_THREADS[@]}"
    do
        CORES=$(seq -s, 0 $((threads - 1)))
        for iter in "${ACTIVE_ITERS[@]}"; do
            taskset -c "$CORES" cargo run --release --features "rayon" rayon $N $threads >> "$OUT" 2>> "$DUMP"
        done
    done

    echo "running NQUEENS velvet"
    for threads in "${ACTIVE_THREADS[@]}"
    do
        CORES=$(seq -s, 0 $((threads - 1)))
        export VELVET_WORKERS=$threads 
        for iter in "${ACTIVE_ITERS[@]}"; do
            taskset -c "$CORES" cargo run --release velvet $N >> "$OUT" 2>> "$DUMP"
        done
    done

    echo "running NQUEENS Velvet with test_direct"
...
if [ "$HAVE_CLANG_OMP" = "true" ]; then
    ...
    clang -O3 "./openmp/$APP/nqueens_seq.c" -lm -o "./zout/${APP}_seq"

    echo "running NQUEENS serial C"
    for iter in "${ACTIVE_ITERS[@]}"; do
        taskset -c 0 "./zout/${APP}_seq" seq $N >> "$OUT" 2>> "$DUMP"
    done

    clang -fopenmp -O3 "./openmp/$APP/${APP}.c" -lm -o "./zout/${APP}_omp"

    echo "running NQUEENS openmp"
    for threads in "${ACTIVE_THREADS[@]}"
    do
        ...
        CORES=$(seq -s, 0 $((threads - 1)))
        for iter in "${ACTIVE_ITERS[@]}"; do
            taskset -c "$CORES" "./zout/${APP}_omp" omp $N >> "$OUT" 2>> "$DUMP"
        done
    done
    cd - > /dev/null
fi
...
if [ "$HAVE_OPENCILK" = "true" ]; then
    ...
    echo "running NQUEENS cilk"
    for threads in "${ACTIVE_THREADS[@]}"
    do
        export CILK_NWORKERS=$threads 
        CORES=$(seq -s, 0 $((threads - 1)))
        for iter in "${ACTIVE_ITERS[@]}"; do
            taskset -c "$CORES" "./zout/${APP}_cilk" cilk $N >> "$OUT" 2>> "$DUMP"
        done
    done
    cd - > /dev/null
fi